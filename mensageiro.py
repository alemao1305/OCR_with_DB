...
def update_output_file(counter, frame_number):
    ...
    if counter.vehicle_count >= 1:
        return  # Sair da função se a condição for atendida
...
"""def enviar_informacoes_para_banco():
    # Coloque aqui o código para enviar as informações para o banco de dados
    data_atual = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    comando_sql = "INSERT INTO Contador_De_Veiculos ('QUANTIDADE', 'DATA_INICIO', 'DATA_FIM') VALUES ({car_counter.vehicle_count}, '{data_inicio}', '{data_atual}');"
    cur.execute(comando_sql)
    con.commit()"""
def enviar_informacoes_para_banco():
    global data_inicio  # Adicione essa linha para garantir que a variável seja usada globalmente
    data_atual = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    comando_sql = f"INSERT INTO Contador_De_Veiculos ('QUANTIDADE', 'DATA_INICIO', 'DATA_FIM') VALUES ({car_counter.vehicle_count}, '{data_inicio}', '{data_atual}');"
    cur.execute(comando_sql)
    con.commit()
...
cur = con.cursor()
 ...
 # Agendar a próxima execução após 5 minutos
def Iniciar_timer_envio():
    threading.Timer(300, enviar_informacoes_para_banco).start()
...
while True:
    ...
    if cv.waitKey(1) == 27:
        ...
        comando_sql = f"INSERT INTO Contador_De_Veiculos ('QUANTIDADE', 'DATA_INICIO', 'DATA_FIM') VALUES ({car_counter.vehicle_count}, '{data_inicio}', '{data_final}');"
        # print(comando_sql)
        cur.execute(comando_sql)
        con.commit()
        break
    ...
    # Adicionar um pequeno atraso para a exibição adequada
    cv.waitKey(10)
